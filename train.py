...
def main():
    try:
        ...
        if val_path and os.path.exists(val_path):
            ...
            train_data, val_data = build_dataset(cfg["dataset_path"], system_prompt, prompt_template, cfg.get("split_ratio", 0.9))

        # Подготовка TrainingArguments с проверкой доступных параметров
        ta_kwargs = {
        ...
        # evaluation strategy
        sig_ta = inspect.signature(TrainingArguments.__init__).parameters
        if "evaluation_strategy" in sig_ta:
            ta_kwargs["evaluation_strategy"] = "steps" if val_data else "no"
        elif "eval_strategy" in sig_ta:
            ta_kwargs["eval_strategy"] = "steps" if val_data else "no"
        if "eval_steps" in sig_ta and val_data:
            ta_kwargs["eval_steps"] = cfg.get("eval_steps")
        ...
        if "max_seq_length" in sig_sft and cfg.get("max_seq_length"):
